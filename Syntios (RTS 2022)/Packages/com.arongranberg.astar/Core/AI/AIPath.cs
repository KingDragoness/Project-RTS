		...
		protected override void MovementUpdateInternal (float deltaTime, out Vector3 nextPosition, out Quaternion nextRotation) {
			...
			if (interpolator.valid) {
				...
				if (!stopped) {
					...
					velocity2D += dir; //TOO EXPENSIVEMovementUtilities.CalculateAccelerationToReachPoint(dir, dir.normalized*maxSpeed, velocity2D, currentAcceleration, rotationSpeed, maxSpeed, forwards) * deltaTime;
				}
			} else {
				reachedEndOfPath = false;
			...
			if (exists_rvoController && rvoController.enabled) {
