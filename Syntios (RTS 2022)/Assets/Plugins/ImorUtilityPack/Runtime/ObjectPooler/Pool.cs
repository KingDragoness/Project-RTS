...
namespace ToolBox.Pools
{
    internal sealed class Pool
    {
        ...
        private static Dictionary<GameObject, Pool> _prefabLookup = new Dictionary<GameObject, Pool>(64);
        private static Dictionary<GameObject, Pool> _instanceLookup = new Dictionary<GameObject, Pool>(512);

        private static GameObject _staticPoolParent;
        public static Transform ParentStaticPool
        {
            get
            {
                if (_staticPoolParent == null)
                {
                    _staticPoolParent = new GameObject();
                    _staticPoolParent.gameObject.name = "PooledObjects";
                }

                return _staticPoolParent.transform;
            }
        }

        public void ClearAllInstance()
        {
            _prefabLookup = new Dictionary<GameObject, Pool>(64);
            _instanceLookup = new Dictionary<GameObject, Pool>(512);
        }
        ...
        private Poolable CreateInstance()
        {
            ...
            instanceGameObject.transform.SetParent(ParentStaticPool);
