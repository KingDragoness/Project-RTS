...
Shader "Syntios/SyntiosTerrain"
{
    Properties
    {
        _SplatMap("Splatmap (Ground,1,2,3,4)", 2D) = "black" {}
        ...
        _BlankSampler("Some Sampler for Normalmap", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            UNITY_DECLARE_TEX2D(_BlankSampler);
        /*  
            UNITY_DECLARE_TEX2D(_TextureC);
            UNITY_DECLARE_TEX2D(_TextureD);
            UNITY_DECLARE_TEX2D(_TextureE);
            UNITY_DECLARE_TEX2D(_TextureF);
            UNITY_DECLARE_TEX2D(_TextureG);
            UNITY_DECLARE_TEX2D(_TextureH);*/
            ...
            UNITY_DECLARE_TEX2D_NOSAMPLER(_NormalMapA);
            UNITY_DECLARE_TEX2D_NOSAMPLER(_NormalMapB);
            UNITY_DECLARE_TEX2D_NOSAMPLER(_NormalMapC);
            UNITY_DECLARE_TEX2D_NOSAMPLER(_NormalMapD);
            UNITY_DECLARE_TEX2D_NOSAMPLER(_NormalMapE);
            UNITY_DECLARE_TEX2D_NOSAMPLER(_NormalMapF);
            UNITY_DECLARE_TEX2D_NOSAMPLER(_NormalMapG);
            UNITY_DECLARE_TEX2D_NOSAMPLER(_NormalMapH);

            //sampler2D _NormalMapB;
            //sampler2D _NormalMapC;
            //sampler2D _NormalMapD;
            //sampler2D _NormalMapE;
            //sampler2D _NormalMapF;
            //sampler2D _NormalMapG;
            //sampler2D _NormalMapH;
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                fixed4 materialHeights_2 = fixed4(materialEColor.a, materialFColor.a, materialGColor.a, materialHColor.a);

                // avoid black artefacts by division by zero
                materialHeights = max(0.0001, materialHeights);
                materialHeights_2 = max(0.0001, materialHeights_2);

                // get material amounts from splatmap
                fixed4 materialAmounts = tex2D(_SplatMap, IN.uvSplat).argb;
                fixed4 materialAmounts_2 = tex2D(_SplatMap2, IN.uvSplat).argb;
                ...
                fixed groundAmount = 1.0 - min(1.0, materialAmounts.r + materialAmounts.g + materialAmounts.b + materialAmounts.a +
                    materialAmounts_2.r + materialAmounts_2.g + materialAmounts_2.b + materialAmounts_2.a);
                ...
                fixed4 alphaMaterials_2 = materialAmounts_2 * IN.materialPrios * materialHeights_2;
                ...
                //splat2
                max_01234 = max(max_01234, alphaMaterials_2.r);
                max_01234 = max(max_01234, alphaMaterials_2.g);
                max_01234 = max(max_01234, alphaMaterials_2.b);
                max_01234 = max(max_01234, alphaMaterials_2.a);
                ...
                fixed b5 = max(alphaMaterials_2.r - max_01234, 0);
                fixed b6 = max(alphaMaterials_2.g - max_01234, 0);
                fixed b7 = max(alphaMaterials_2.b - max_01234, 0);
                fixed b8 = max(alphaMaterials_2.a - max_01234, 0);
                ...
                //NORMALMAP
                {
                    ...
                    float4 texN = (
                        tex2DStochastic(UNITY_SAMPLE_TEX2D_SAMPLER(_NormalMapA, _BlankSampler, IN.uvMaterial), IN.uvMaterial) * b1 +
                        tex2DStochastic(UNITY_SAMPLE_TEX2D_SAMPLER(_NormalMapB, _BlankSampler, IN.uvMaterial), IN.uvMaterial) * b2 +
                        tex2DStochastic(UNITY_SAMPLE_TEX2D_SAMPLER(_NormalMapC, _BlankSampler, IN.uvMaterial), IN.uvMaterial) * b3 +
                        tex2DStochastic(UNITY_SAMPLE_TEX2D_SAMPLER(_NormalMapD, _BlankSampler, IN.uvMaterial), IN.uvMaterial) * b4 +
                        tex2DStochastic(UNITY_SAMPLE_TEX2D_SAMPLER(_NormalMapE, _BlankSampler, IN.uvMaterial), IN.uvMaterial) * b5 +
                        tex2DStochastic(UNITY_SAMPLE_TEX2D_SAMPLER(_NormalMapF, _BlankSampler, IN.uvMaterial), IN.uvMaterial) * b6 +
                        tex2DStochastic(UNITY_SAMPLE_TEX2D_SAMPLER(_NormalMapG, _BlankSampler, IN.uvMaterial), IN.uvMaterial) * b7 +
                        tex2DStochastic(UNITY_SAMPLE_TEX2D_SAMPLER(_NormalMapH, _BlankSampler, IN.uvMaterial), IN.uvMaterial) * b8
                        ...
                        tex2DStochastic(_NormalMapE, IN.uvMaterial) * b5 +
                        tex2DStochastic(_NormalMapF, IN.uvMaterial) * b6 +
                        tex2DStochastic(_NormalMapG, IN.uvMaterial) * b7 +
                        tex2DStochastic(_NormalMapH, IN.uvMaterial) * b8*/
                        );
        ...
        Pass
        {
            ...
            v2f vert(a2v v)
            {
                v2f OUT;
                OUT.pos = UnityObjectToClipPos(v.vertex);
                OUT.uvSplat = mul(unity_ObjectToWorld, v.vertex).xz * _SplatmapScale * 0.01;
                OUT.uvFOW = mul(unity_ObjectToWorld, v.vertex).xz * _FOWmapScale * _SplatmapScale * 0.01; 
            ...
            {
                // store heights for all materials on this pixel
                fixed groundHeight = 0;
                fixed4 materialHeights = fixed4(1,1,1,1);
                fixed4 materialHeights_2 = fixed4(1,1,1,1);
                ...
                fixed4 col = tex2Dproj(_LastPass, IN.grabPos);
